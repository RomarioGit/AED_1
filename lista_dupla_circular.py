##########################################################$
# Lista Duplamente Circular Ligada                         #
#   - Implementação próxima a lista built-in do Python     #
# Prof. Ygor Amaral - UFRPE/UAST                           #
# Alunos: Thiago, Romário, José, Matheus H., Ledson, Lucas #
###########################################################$
...
class Lista:
    ...
    def _next_(self):
        if self.__iterando is None:
            self._iterando = self._primeiro  # pegando o primeiro sempre atualizado
        else:
            self._iterando = self._iterando.próximo
            ...
            while i <= item:
                ...
                if i == item:
                    if anterior is None:
                        ...
                        atual.próximo = self.__primeiro
                        atual.próximo.anterior = self.__último
                        print("Apagou o primeiro")
                        break
                        #atual = None

                    elif atual.próximo is self.__primeiro:
                        # apagando o último elemento da lista
                        atual.anterior = anterior
                        self.__último = anterior
                        anterior.proximo = self.__primeiro
                        ...
                        atual.próximo.anterior = anterior
    ...
    def inserir(self, índice, conteúdo):
        ...
        if self.__primeiro is None:
            ...
            #FAZENDO ELA SER CIRCULAR
            novo.proximo = self.__primeiro
            novo.anterior = self.__último
...
lista = Lista()
lista.inserir_início('Thiago')
lista.inserir_fim('Romário')
lista.inserir(1,'stark')
lista.inserir(1,'José')
print(lista)
#lista.remover('stark')
#print(lista)
#lista.reverso()
